...
async function fct (message, args, client, db, tools) {
    ...
    else if (memberInfo.lastClaim < (24 * 3600000 + date)) return message.channel.send(`Il est encore trop tôt pour récupérer vos bolducs, revenez dans ${tools.howManyLast (date, memberInfo.lastClaim)}.`)
    ...
    client.channels.cache.get('804480347592589312').send(`${message.channel.tag} a gagné ${amount} bolducs avec la commande claim.`)
}