...
async function fct (message, args, client, db) {
    ...
    message.channel.send(`Vous possédez actuellement ${inventory.bolducs} Bolduc${inventory.bolducs > 1 ? 's' : ''} <:1B:805427963972943882> (${diff > 0 ? '+' :''}${diff} / augmentation de ${inventory.bolducs * 100 / (inventory.bolducs - diff)}% par rapport à hier)`)
}